// ==========================================
// LatinaTapes - Simplified Application Script
// No JWT. Basic Auth + Obfuscated Creds.
// ==========================================
...
const CONFIG = {
    ...
    // These are base64 encoded. To change them:
    // 1. Open browser console
    // 2. Run: btoa('your_username')  and  btoa('your_password')
    // 3. Paste the results below
    _u: 'a2Vuem5vdnU=',      // btoa('kenznovu')
    _p: 'SHVoMTMxWGprMDk=',  // btoa('Huh131Xjk09')

    get ADMIN_USER() { return atob(this._u); },
    get ADMIN_PASS() { return atob(this._p); },
...
const VideoDB = {
    ...
    async add(video) {
        if (!Auth.check()) throw new Error('Admin not authenticated');

        const creds = btoa(`${CONFIG.ADMIN_USER}:${CONFIG.ADMIN_PASS}`);

        // 1. Get presigned URL from Worker for the video
        ...
        const uploadRes = await fetch(presignData.uploadUrl, { method: 'PUT', body: video.videoFile });
        if (!uploadRes.ok) throw new Error('Video upload to R2 failed');
        ...
        if (video.thumbnailFile) {
            ...
            const thumbUpload = await fetch(thumbData.uploadUrl, { method: 'PUT', body: video.thumbnailFile });
            if (!thumbUpload.ok) throw new Error('Thumbnail upload to R2 failed');
...
// ==========================================
// AUTHENTICATION (No JWT — Simple Session)
// ==========================================
const Auth = {
    login(username, password) {
        // Decode obfuscated credentials and compare
...
function renderVideoGrid(videos) {
    ...
    if (videos.length === 0) {
        grid.innerHTML = `<div style="grid-column: 1 / -1; text-align: center; padding: 60px 20px; color: var(--text-secondary);">
            <h3>No videos yet</h3><p>Upload your first video from the admin panel.</p>
        </div>`;
        return;
    }
...
function setupSortDropdown(currentSort) {
    ...
    filterDropdown.querySelectorAll('a').forEach(link => {
        ...
        if (link.dataset.sort === currentSort) link.classList.add('active-sort');
    });
    ...
    window.addEventListener('click', () => filterDropdown.classList.remove('active'));
}
...
function setupSearch() {
    ...
    function performSearch() {
        ...
        const results = VideoDB.videos.filter(v => v.title.toLowerCase().includes(query));
        const grid = document.querySelector('.video-grid');
